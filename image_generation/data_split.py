...
image_dir = '/dtu/blackhole/07/203495/ADLCV-project/data/ISIC_2019_Training_Input'
metadata_path = '/dtu/blackhole/07/203495/ADLCV-project/data/ISIC_2019_Training_Input/ISIC_2019_Training_Metadata.csv'
ground_truth_path = '/dtu/blackhole/07/203495/ADLCV-project/data/ISIC_2019_Training_Input/ISIC_2019_Training_GroundTruth.csv'
generated_descriptions_path = '/dtu/blackhole/07/203495/ADLCV-project/data/ISIC_2019_Training_Input/generated_descriptions_isic_full_v2.csv'
output_dir = '/dtu/blackhole/07/203495/ADLCV-project/data/ISIC_2019_Training_Input_split'
...
descriptions = pd.read_csv(generated_descriptions_path, delimiter='\t')  # Use tab delimiter
...
def save_split(df, split_name):
    ...
    # Save descriptions portion
    # Try both 'image' and 'image_id' as possible keys
    if 'image_id' in descriptions.columns:
        desc_key = 'image_id'
    else:
        desc_key = 'image'
    split_desc = descriptions[descriptions[desc_key].isin(df['image'])]
    split_desc.to_csv(os.path.join(output_dir, f'{split_name}_descriptions.csv'), index=False, sep='\t')  # Write as tab-separated
